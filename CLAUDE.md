...
- **Guides** — Docker/Compose, venv + Django, React/Vite, PostgreSQL, Redis,
  GitHub Actions, a JWT refresh-race writeup, and a filterable index of the
  Codewars solutions.
...
├── data/codewars.js       GENERATED by scripts/gen-codewars.mjs — do not hand-edit
...
├── features/guides/solution-list.js the one stateful block renderer (category filter)
...
Guide blocks are
`h2 | h3 | p | ul | code | note | warn | image | commands | links | solutions`.
...
`data/codewars.js` is generated from the sibling `data-structures-practice`
checkout by `client/scripts/gen-codewars.mjs` and **committed**, so the build
never reaches outside this repo. Rerun after adding katas:

```
cd client
npm run gen:codewars              # or CODEWARS_DIR=/path/to/codewars npm run gen:codewars
```

Categories live in `client/scripts/categories.json`, keyed by solution filename
without `.py`. The generator fails loudly if the two sides drift — a solution
with no category, or a category with no file — rather than emitting a partial
page.

`solutions` is the only block type that holds state, so it lives in its own
module rather than inline in `guide-view.js`. Two things it does deliberately:
the filter hides rows with the `hidden` property (so hidden rows leave the tab
order), and each row's code block is built on **first expand** — highlighting
191 blocks up front is a visible hitch, and most are never opened.
